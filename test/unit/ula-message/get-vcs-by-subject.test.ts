...
describe('get-vcs-by-subject ULA message', function () {
  ...
  it('should return the correct VC', (done) => {
    ...
    const vcRepoStub = sinon.stub(vcRepo, 'findByCredentialSubject').resolves(repositoryResult)
    const regex = /schema\.org\/givenname/i
    const ulaTestMessage = new Message({
      type: 'get-vcs-by-subject',
      contextRegex: regex
    })
