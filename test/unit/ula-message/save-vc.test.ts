...
describe('save-vc ULA message', function () {
  ...
  it('should save a new VerifiableCredential properly', () => {
    const vcRepoStub = sinon.stub(vcRepo, 'saveMultiple')
    const verifiableCredentialArr = [new VerifiableCredential(testCredParams)]
    const ulaTestMessage = new Message({
      type: 'save-vcs',
      verifiableCredentials: verifiableCredentialArr
    })
    ...
    vcRepoStub.should.have.been.calledOnceWithExactly(verifiableCredentialArr)
